...
--------+---------------------------+-------+----------
 ...
 public | people_roles              | table | postgres <-- db_people_roles()
...
from swagger_server.response_code.comanage_utils import update_organizations
...
def db_people_roles() -> None:
    ...
    for fab_role in fab_roles:
        try:
            ...
            logger.info(
                "- Checking: CoPersonRole {0} for entry in 'people_roles' table".format(fab_role.co_person_role_id))
            if compare_role_to_coperson_role(fab_role=fab_role, co_role=co_role) and str(
                    co_role.get('Deleted')).casefold() != 'true':
                ...
                if str(co_role.get('Deleted')).casefold() == 'true':
                    logger.info("  - *** DELETE ***: field mismatch - delete - {0}".format(fab_role.name))
                    logger.info("  - fields: status: {0}, deleted: {1}".format(co_role.get('Status'),
                                                                               str(co_role.get('Deleted'))))
                    FabricRoles.query.filter_by(id=fab_role.id).delete()
                    db.session.commit()
                # otherwise update role to match CoPersonRole
                else:
                    logger.info("  - *** UPDATE ***: field mismatch - update - {0}".format(fab_role.name))
                    logger.info("  - fields: update all - status {0}".format(co_role.get('Status')))
            ...
            # cannot find corresponding CoPersonRole - potential error
            logger.info("  - *** ERROR ***: role not found, reason: {0}".format(exc))
