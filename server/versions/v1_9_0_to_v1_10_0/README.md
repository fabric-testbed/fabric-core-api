...
| `pre_flight_creator_owner_report.py` | helper script (read-only) | Pre-flight generator for `project_creator_owners.md`. Runs against the live v1.9.0 deployment via `docker exec` so operators can reconcile creator/owner mismatches BEFORE the migration starts — see Phase 0a below |
...
### 0a. (Optional but recommended) Creator/owner reconciliation against v1.9.0

Phase 5a normally handles creator-vs-owner reconciliation after the migration is complete. Running the same review **before** Phase 0's backup means the captured archive — and every downstream artifact — already reflects the corrected ownership state, so the post-migration Phase 5a is a no-op.
...
For each bolded row, reconcile via the live v1.9.0 portal/API (`PATCH /projects/{uuid}/project-owners`, operation `add`, with the creator's UUID) the same way you would in Phase 5a. After fixes land, re-run the helper to confirm zero mismatches.

When done, you can proceed to 0b below. If you skip 0a entirely, Phase 5a (post-migration) is the fallback.

### 0b. Full backup (REQUIRED)
...
> If you ran Phase 0a (optional pre-flight reconciliation) before the backup, all rows should already read **Yes** / **N/A** and this section is a no-op. Spot-check to confirm, then proceed to 5b.
...
| Phase | Action | Type |
|---|---|---|
| 0a | (Optional) `pre_flight_creator_owner_report.py` → reconcile creator/owner via v1.9.0 API | Manual |
| 0b | `full-backup.sh` + COU snapshot | Manual |
